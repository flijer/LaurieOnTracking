...
def plot_frame(hometeam, awayteam, pass_team, player_with_ball, figax=None, team_colors=('r', 'b'), field_dimen=(106.0, 68.0),
               include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False, offside=True):
    ...
    # TODO Plot offside line #away ---> blue, #home <--- red
    # home_team_max_x = hometeam[hometeam[list([c for c in hometeam.keys() if c[-2:].lower()=='_x' and c!='ball_x'])].idxmax()]

    # attacking team
    players_in_offside_x = []
    players_in_offside_y = []
    if offside:
        if pass_team == 'Home':  # Attacking team is home <---

            second_to_last_defender_x = awayteam[list([c for c in awayteam.keys() if c[-2:].lower() == '_x' and c != 'ball_x'])].sort_values(ascending=True)[1]  # Min x
            attacking_players = hometeam[list([c for c in hometeam.keys() if c[-2:].lower() == '_x' and c != 'ball_x'])]
            players_in_offside_x = attacking_players.where(attacking_players < second_to_last_defender_x + 5)  # players currenctly in offside

        else:  # attacking team away (blue) -->
            second_to_last_defender_x = hometeam[list([c for c in hometeam.keys() if c[-2:].lower() == '_x' and c != 'ball_x'])].sort_values( ascending=False)[1]  # Max x
            # attackers
            attacking_players = awayteam[list([c for c in awayteam.keys() if c[-2:].lower() == '_x' and c != 'ball_x'])]

            players_in_offside_x = attacking_players.where(attacking_players > second_to_last_defender_x - 5)  # players currenctly in offside

        players_in_offside_x = players_in_offside_x.dropna().index.to_list()
        players_in_offside_y = [x.replace('x', 'y') for x in players_in_offside_x]  # players currenctly in offside y

        #Offside line
        objs, = ax.plot([second_to_last_defender_x, second_to_last_defender_x], [-34, 34], 'yellow',linewidth=3)  # x,x, y,y
        figobjs.append(objs)

    for team, color in zip([hometeam, awayteam], team_colors):

        x_columns = [c for c in team.keys() if c[-2:].lower() == '_x' and c != 'ball_x']# and c not in players_in_offside_x]  # column header for player x positions
        y_columns = [c for c in team.keys() if c[-2:].lower() == '_y' and c != 'ball_y']# and c not in players_in_offside_y]  # column header for player y positions

        objs, = ax.plot(team[x_columns], team[y_columns], color + 'o', MarkerSize=PlayerMarkerSize,
                alpha=PlayerAlpha)  # plot player positions
        figobjs.append(objs)

        # Different color for players in offside
        if (offside is True) and (len(players_in_offside_x) > 0) and (players_in_offside_x[0] in team):
            objs = ax.plot(team[players_in_offside_x], team[players_in_offside_y], 'y' + 'o', MarkerSize=PlayerMarkerSize,
                    alpha=PlayerAlpha)  # plot player positions
            figobjs.append(objs)
        # marked player in offside
        ...
        if annotate:
            ...
            # plot ball
    objs, = ax.plot(hometeam['ball_x'], hometeam['ball_y'], 'ko', MarkerSize=6, alpha=1.0, LineWidth=0)
    figobjs.append(objs)
    return fig, ax, figobjs


def save_match_clip_pitch_control(frame_from, frame_to,events,tracking_home, tracking_away, params, fpath='',fname='',frames_per_second=25,field_dimen=(106.0, 68.0),figax=None,video_type='mp4',dpi=50):

    # Set figure and movie settings
    # FFMpegWriter = animation.writers['ffmpeg'] #imagemagick ffmpeg
    # ImageMagickWriter = animation.writers['imagemagick']
    ...
    if video_type == 'mp4':
        ...
        plt.rcParams['animation.convert_path'] = 'C:/Program Files/ImageMagick/magick.exe'
        writer = animation.ImageMagickWriter(fps=frames_per_second, metadata=metadata)

    fname = fpath + '/' + fname + '.'+video_type  # path and filename
    ...
    with writer.saving(fig, fname, dpi=dpi):
        for i in range(frame_from,frame_to):
            ...
            PPCF, xgrid, ygrid = mpc.generate_pitch_control_for_event(i, events, tracking_home, tracking_away, params,
                                                                      field_dimen=(106., 68.,), n_grid_cells_x=25,animate=True)
            print("--- %s seconds ---" % (time.time() - start_time))

            _,_,figobjs = plot_pitchcontrol_for_event(i, events, tracking_home, tracking_away, PPCF, xgrid, ygrid, annotate=False, figax=(fig, ax),animate=True)

            frame_minute = int(tracking_home.loc[i]['Time [s]'] / 60.)
            frame_second = (tracking_home.loc[i]['Time [s]'] / 60. - frame_minute) * 60.
    ...
    for i, row in events.iterrows():
        ...
        if 'Arrow' in indicators:
            ax.annotate("", xy=row[['End X', 'End Y']], xytext=row[['Start X', 'Start Y']], alpha=alpha,
                        arrowprops=dict(alpha=alpha, arrowstyle="->", color=color), annotation_clip=False)
        if annotate:
...
def plot_pitchcontrol_for_event(event_id, events, tracking_home, tracking_away, PPCF, xgrid, ygrid, alpha=0.7,
                                include_player_velocities=True, annotate=False, field_dimen=(106.0, 68), figax=None,animate=False):
    ...
    # player_with_ball = events.loc[event_id].From
    player_with_ball = ''
    if not animate:
    ...
    if figax is None:
        ...
        fig, ax = figax
    _,_, figobjs = plot_frame(tracking_home.loc[pass_frame], tracking_away.loc[pass_frame], pass_team, player_with_ball, figax=(fig, ax),
               PlayerAlpha=alpha, include_player_velocities=include_player_velocities, annotate=annotate, offside=True)
    ...
    if pass_team == 'Home':
        ...
        cmap = 'bwr_r'
    objs = ax.imshow(np.flipud(PPCF), extent=(np.amin(xgrid), np.amax(xgrid), np.amin(ygrid), np.amax(ygrid)),
              interpolation='hanning', vmin=0.0, vmax=1.0, cmap=cmap, alpha=0.5)
    figobjs.append(objs)
    # merged = list(itertools.chain.from_iterable(figobjs))
    return fig, ax, figobjs
